  ...
  async initAdminUsernameUser(
    ...
    if (userCount === 0) {
      ...
      try {
        ...
        // Only update password if user has admin role
        if (rootRole.name === RoleName.ADMIN) {
          this.logger.info(
            `Updating password for existing admin user '${username}'`
          );
          const passwordHash = await bcrypt.hash(password, saltRounds);
          await this.store.setPasswordHash(
            existingUser.id,
          ...
          this.logger.info(
            `User '${username}' exists but is not an admin, skipping password update`
          );
        }
      } catch (e) {
        // User doesn't exist or other error - this is expected if username doesn't match
        this.logger.debug(
